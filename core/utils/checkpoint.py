...
class Checkpoint():
  ...
  def get_latest_chkp_path(self, chkp_fold_path):
    ckpt = tf.train.get_checkpoint_state(chkp_fold_path)
    return ckpt.model_checkpoint_path

  def write_checkpoint_model_items(self, chkp_file_path, chkp_model_list):
  ...
  def get_checkpoint_model_items(self, chkp_file_path):
  ...
  def print_checkpoint_variables(self, filepath):
  ...
  def print_checkpoint_specific_tensor(self, filepath, name):
    reader = pywrap_tensorflow.NewCheckpointReader(filepath)
    if reader.has_tensor(name):
      print("tensor_name: ", name)
      print(reader.get_tensor(name))

  def change_checkpoint_prefix(self, infile, outfile, old_prefix, new_prefix):
  ...
  def add_checkpoint_prefix(self, infile, outfile, new_prefix):
    """ Solve the issue of different name scope of same model.
    e.g.
        # change_checkpoint_prefix(
        #     infile='vgg_16.ckpt', outfile='vgg_16_new',
  ...
  def merge_checkpoint(self, infile_src, infile_dst, outfile):
  ...
  def merge_fuse_checkpoint(self, model_1, model_2, fuse_model, outfile):
    """
    e.g.
    ...
    self.change_checkpoint_prefix(model_1, model_1 + '_1', 'net', 'net1')
    print('\n Changing the prefix of model2')
    self.change_checkpoint_prefix(model_2, model_2 + '_2', 'net', 'net2')
    print('\n Merge Model1')
    self.merge_checkpoint(model_1 + '_1', fuse_model, fuse_model + '_tmp')
    print('\n Merge Model2')
    self.merge_checkpoint(model_2 + '_2', fuse_model + '_tmp', outfile)
    print('\n Finished!')

  def compute_parameter_num(self, filepath):
