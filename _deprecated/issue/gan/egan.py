...
class EGAN():
    ...
    def __init__(self, dataset, is_training=True):
        ...
        # data related
        self.batch_size = int(dataset.batch_size / 2)
        ...
        if self.output_height == 128:
            ...
            self.encoder = self.encoder_128x128
    ...
    def model(self, global_step, inputs, labels):
        ...
        if self.one_hot:
            self.Y1 = tf.to_float(Y1)
            self.Y2 = tf.to_float(Y2)
        else:
            self.Y1 = tf.to_float(tf.one_hot(Y1, depth=self.Y_dim, on_value=1))
            self.Y2 = tf.to_float(tf.one_hot(Y2, depth=self.y_dim, on_value=1))
        ...
        # fake
        self.E = self.encoder(self.X1, self.Y1, False)
        self.G = self.generator(self.E, self.Y1, reuse=False)
        self.D_, self.D_logits_ = self.discriminator(
            self.G, self.Y1, reuse=False)
        # true
        self.D, self.D_logits = self.discriminator(
            self.X2, self.Y2, reuse=True)
        ...
        self.g_vars = [var for var in t_vars if 'generator' or 'encoder' in var.name]
    ...
    def encoder_128x128(self, image, y, reuse=False, name='encoder'):
        with tf.variable_scope(name) as scope:
            if reuse:
                scope.reuse_variables()

            with tf.variable_scope('e_block_init'):
                yb = tf.reshape(y, [self.batch_size, 1, 1, self.y_dim])
                x = self.conv_cond_concat(image, yb, name='e_concat_x')

            # 128x128
            with tf.variable_scope('e_block0'):
                h0_conv = self.conv2d(
                    x, self.c_dim + self.y_dim, name='e_conv0')
                h0 = self.leak_relu(h0_conv)
                h0 = self.conv_cond_concat(h0, yb, 'e_cond_concat_0')

            # 64x64
            with tf.variable_scope('e_block1'):
                h1_conv = self.conv2d(h0, 64 + self.y_dim, name='e_conv1')
                h1 = self.leak_relu(self.batch_norm(h1_conv))
                h1 = self.conv_cond_concat(h1, yb, 'e_cond_concat_1')

            # 32x32
            with tf.variable_scope('e_block2'):
                h2_conv = self.conv2d(h1, 92 + self.y_dim, name='e_conv2')
                h2 = self.leak_relu(self.batch_norm(h2_conv))
                h2 = self.conv_cond_concat(h2, yb, 'e_cond_concat_2')

            # 16x16
            with tf.variable_scope('e_block3'):
                h3_conv = self.conv2d(h2, 128 + self.y_dim, name='e_conv3')
                h3 = self.leak_relu(self.batch_norm(h3_conv))
                h3 = self.conv_cond_concat(h3, yb, 'e_cond_concat_3')

            # 8x8
            with tf.variable_scope('e_block4'):
                h4_conv = self.conv2d(h3, 256 + self.y_dim, name='e_conv4')
                h4 = self.leak_relu(self.batch_norm(h4_conv))
                h4 = self.conv_cond_concat(h4, yb, 'e_cond_concat_4')

            # 4x4
            with tf.variable_scope('e_block5'):
                h5_conv = self.conv2d(h4, 256 + self.y_dim, name='e_conv5')
                h5 = self.leak_relu(self.batch_norm(h5_conv))
                h5 = tf.reshape(h5, [self.batch_size, -1])

            # linear1
            with tf.variable_scope('e_block6'):
                return self.linear(h5, 100, name='e_fc_6')
...
def train(data_name, chkp_path=None):
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope('dataset'):
            ...
            X1, X2 = tf.split(images, 2, axis=0)
            Y1, Y2 = tf.split(labels, 2, axis=0)

        # -------------------------------------------
        # GAN
        # -------------------------------------------
        global_step = framework.create_global_step()
        gan = EGAN(dataset, 'train')
        train_op = gan.model(global_step, [X1, X2], [Y1, Y2])
