...
def train():
    mnist = input_data.read_data_sets('../_output/MNIST_data', one_hot=True)
    mb_size = 64
    z_dim = 100
    ...
    # Gaussian
    kl_loss = 0.5 * \
        tf.reduce_sum(tf.exp(z_logvar) + z_mu**2 - 1. - z_logvar, 1)
    # VAE loss
    vae_loss = tf.reduce_mean(recon_loss + kl_loss)

    solver = tf.train.AdamOptimizer(0.0002).minimize(vae_loss)
