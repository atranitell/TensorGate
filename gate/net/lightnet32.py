...
class Lightnet32(net.Net):

    default_image_size = 36
    ...
    def model(self, images, num_classes, is_training, scope='lightnet32'):

        end_points = {}

        with tf.variable_scope(scope, 'lightnet32', [images, num_classes]):

            with arg_scope([layers.batch_norm], is_training=is_training):

                block_in = layers.conv2d(images, 64, [7, 7], 1)
                block_in = layers.max_pool2d(block_in, [3, 3], 1)
            ...
            end_points['gap_conv'] = block_in
            block_in = layers.dropout(
                block_in, keep_prob=self.dropout, is_training=is_training)
            block_in = layers.avg_pool2d(block_in, [8, 8], 1, padding='VALID')
            block_in = layers.flatten(block_in)
            end_points['gap_pool'] = block_in
