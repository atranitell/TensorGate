...
def detection_head_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
  ...
  """
  n_roi_img = config.TRAIN_ROIS_PER_IMAGE
  names = ["rois", "target_class_ids", "target_bbox", "target_mask"]
  rois, class_ids, bbox, mask = utils.batch_slice(
      [proposals, gt_class_ids, gt_boxes, gt_masks],
      lambda w, x, y, z: detection_head(w, x, y, z, config),
      config.BATCH_SIZE, names=names)
  rois = tf.reshape(rois, [-1, n_roi_img, 4])
  class_ids = tf.reshape(class_ids, [-1, n_roi_img])
  bbox = tf.reshape(bbox, [-1, n_roi_img, 4])
  mask = tf.reshape(mask, [-1, n_roi_img] + config.MASK_SHAPE)
  return rois, class_ids, bbox, mask


def detection_head(proposals, gt_class_ids, gt_boxes, gt_masks, config):
