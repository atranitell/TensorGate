# -*- coding: utf-8 -*-
""" regression cnn video task for image
    updated: 2017/06/09
...
def get_network(images, dataset, phase, scope=''):
    # transpose images
    # print(images)
    images = tf.reshape(images, [dataset.image.output_height,
                                 dataset.image.output_width,
                                 dataset.image.frames,
    ...
    lstm_cell = rnn.BasicLSTMCell(
        n_hidden, forget_bias=1.0, activation=tf.nn.relu)

    # get lstm cell output
    lstm_outputs, states = rnn.static_rnn(lstm_cell, X, dtype=tf.float32)
