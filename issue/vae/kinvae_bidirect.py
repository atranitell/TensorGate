...
class KIN_VAE_BIDIRECT(context.Context):
  """ """
  ...
  def train(self):
    ...
    # discriminator
    D_c1_fake, _ = self._discriminator(c1_fake, cond, reuse=False)
    D_p1_real, _ = self._discriminator(p1_real, cond, reuse=True)

    D_p2_fake, _ = self._discriminator(p2_fake, cond, reuse=True)
    D_c2_real, _ = self._discriminator(c2_real, cond, reuse=True)
  ...
  def _val_or_test(self, dstdir):
    ...
    with tf.Session() as sess:
      ...
      info = utils.string.concat(
          batchsize, [c1_path, p1_path, c2_path, p2_path, label, loss])
