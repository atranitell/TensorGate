...
def get_network(x1, x2, y1, y2, labels, dataset, phase):
    ...
    logits_x1, end_points_x1 = gate.net.factory.get_network(
        dataset.hps, phase, data1, 1, 'net1')
    logits_y1, end_points_y1 = gate.net.factory.get_network(
        dataset.hps, phase, data2, 1, 'net2')

    logits_x2, end_points1_1 = gate.net.factory.get_network(
        dataset.hps, phase, x2, 1, 'net1_1')
    logits_y2, end_points2_1 = gate.net.factory.get_network(
        dataset.hps, phase, y2, 1, 'net2_1')

    logits_x = tf.concat([logits_x1, logits_x2], axis=1)
    logits_y = tf.concat([logits_y1, logits_y2], axis=1)

    is_training = True if phase is 'train' else False
    losses, predictions = gate.loss.cosine.get_loss(
        logits_x, logits_y, labels, dataset.batch_size, is_training)
...
def train(data_name, chkp_path=None, exclusions=None):
    ...
    with tf.Graph().as_default():
        ...
        losses, predications = get_network(
            x1, x2, y1, y2, labels, dataset, 'train')
...
def test(data_name, chkp_path, threshold, summary_writer=None):
    ...
    with tf.Graph().as_default():
        ...
        losses, predictions = get_network(
            x1, x2, y1, y2, labels, dataset, 'test')
